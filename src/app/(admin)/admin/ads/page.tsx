    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-[#E8DDD0]/60">{ads.length} total ads</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-[#E8DDD0]/60">Active Ads</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">{ads.filter((a) => a.isActive).length}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-[#E8DDD0]/60">Total Impressions</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">{ads.reduce((sum, a) => sum + (a.impressions || 0), 0).toLocaleString()}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-[#FF6B00]/10 flex items-center justify-center">
              <MousePointer className="h-5 w-5 text-[#FF6B00]" />
            </div>
            <div>
              <p className="text-xs text-gray-500 dark:text-[#E8DDD0]/60">Total Clicks</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">{ads.reduce((sum, a) => sum + (a.clicks || 0), 0).toLocaleString()}</p>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-[#241C16] border-b border-gray-200 dark:border-[#D4AF37]/10 sticky top-0">
                <tr>
                  ...
                  <th className="text-left font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">Position</th>
                  <th className="text-right font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">Clicks</th>
                  <th className="text-right font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">Impressions</th>
                  <th className="text-right font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">CTR</th>
                  <th className="text-left font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">Status</th>
                  <th className="text-left font-medium text-gray-500 dark:text-[#E8DDD0]/60 px-4 py-3">Date Range</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-12 text-gray-400 dark:text-[#E8DDD0]/40">
                      <Megaphone className="h-8 w-8 mx-auto mb-2 text-gray-300 dark:text-[#E8DDD0]/30" />
                      No ads found.
                    </td>
                  </tr>
                ) : (
                  paged.map((ad) => (
                    <tr key={ad.id} className="hover:bg-gray-50 dark:hover:bg-[#241C16] transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          {ad.imageUrl ? (
                            <img src={ad.imageUrl} alt={ad.title} className="h-10 w-14 rounded object-cover" />
                          ) : (
                            <div className="h-10 w-14 rounded bg-gray-100 dark:bg-[#241C16] flex items-center justify-center">
                              <Megaphone className="h-4 w-4 text-gray-400 dark:text-[#E8DDD0]/40" />
                            </div>
                          )}
                          <div>
                            <p className="font-medium text-gray-900 dark:text-white">{ad.title}</p>
                            {ad.page && <p className="text-xs text-gray-400 dark:text-[#E8DDD0]/40">{ad.page}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3"><Badge variant="outline">{ad.position}</Badge></td>
                      <td className="px-4 py-3 text-right text-gray-900 dark:text-white font-medium">{ad.clicks.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right text-gray-600 dark:text-[#E8DDD0]/70">{ad.impressions.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right font-medium text-[#FF6B00]">{getCTR(ad.clicks, ad.impressions)}</td>
                      <td className="px-4 py-3">
                        {ad.isActive ? <Badge variant="green">Active</Badge> : <Badge variant="red">Inactive</Badge>}
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-[#E8DDD0]/60 text-xs">
                        {ad.startDate ? dayjs(ad.startDate).format('DD MMM') : '-'}
                        {ad.endDate ? ` - ${dayjs(ad.endDate).format('DD MMM')}` : ''}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => toggleAdActive(ad)} className="p-1 hover:bg-gray-100 dark:hover:bg-[#241C16] rounded">
                            {ad.isActive
                              ? <ToggleRight className="h-4 w-4 text-green-600" />
                              : <ToggleLeft className="h-4 w-4 text-gray-400 dark:text-[#E8DDD0]/40" />
                            }
                          </button>
                          <button onClick={() => openEditModal(ad)} className="p-1 hover:bg-gray-100 dark:hover:bg-[#241C16] rounded text-gray-400 dark:text-[#E8DDD0]/40 hover:text-[#FF6B00]">
                            <Edit className="h-4 w-4" />
                          </button>
                          <button onClick={() => setSelectedAd(ad)} className="p-1 hover:bg-gray-100 dark:hover:bg-[#241C16] rounded text-gray-400 dark:text-[#E8DDD0]/40 hover:text-gray-600 dark:hover:text-[#E8DDD0]/70">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedAd(null)}>
          <div className="bg-white dark:bg-[#1A1210] rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-[#D4AF37]/10">
              ...
              <button onClick={() => setSelectedAd(null)} className="text-gray-400 dark:text-[#E8DDD0]/40 hover:text-gray-600 dark:hover:text-[#E8DDD0]/70">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowFormModal(false)}>
          <div className="bg-white dark:bg-[#1A1210] rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-[#D4AF37]/10">
              ...
              <button onClick={() => setShowFormModal(false)} className="text-gray-400 dark:text-[#E8DDD0]/40 hover:text-gray-600 dark:hover:text-[#E8DDD0]/70">
                <XCircle className="h-5 w-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-[#E8DDD0]/80 mb-1.5">Position</label>
                  <select
                    value={formPosition}
                    onChange={(e) => setFormPosition(e.target.value)}
                    className="w-full rounded-lg border border-gray-300 dark:border-[#D4AF37]/15 bg-white dark:bg-[#241C16] px-3 py-2.5 text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-[#FF6B00]/20 dark:focus:ring-[#D4AF37]/20 focus:border-[#FF6B00] dark:focus:border-[#D4AF37]"
                  >
