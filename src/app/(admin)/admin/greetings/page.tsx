    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{greetings.length} greeting cards</p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-400">No greetings found</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((greeting) => (
            <Card key={greeting.id} className="overflow-hidden">
              {greeting.imageUrl ? (
                ...
                <div className="w-full h-40 bg-gradient-to-br from-[#E07B39]/20 to-[#D4A017]/20 flex items-center justify-center">
                  <ImageIcon className="h-12 w-12 text-[#E07B39]/40" />
                </div>
              )}
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-semibold text-gray-900 text-sm">{greeting.title}</h3>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => openEditModal(greeting)} className="text-gray-400 hover:text-[#E07B39]">
                      <Edit className="h-4 w-4" />
                    </button>
                  </div>
                </div>
                <p className="text-xs text-gray-500 line-clamp-2 mb-3">{greeting.message}</p>
