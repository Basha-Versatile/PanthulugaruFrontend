...
import { getMasterRituals, updateRitual } from '@/lib/api/admin';
...
export default function RitualDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await updateRitual(id, updateData as any);
      if (res.success) {
        toast.success('Ritual updated successfully');
      } else {
        toast.error(res.message || 'Failed to save ritual');
      }
    } catch {
  ...
  if (!ritual) {
    return (
      <div className="text-center py-16">
        <p className="text-gray-500 dark:text-[#E8DDD0]/60 mb-4">Ritual not found</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{ritual.name}</h1>
            <p className="text-sm text-gray-500 dark:text-[#E8DDD0]/60">Ritual ID: {ritual.id}</p>
          </div>
        </div>
              ...
              {ritual.subRituals && ritual.subRituals.length > 0 ? (
                <div className="space-y-3">
                  {ritual.subRituals.map((sr) => (
                    <div key={sr.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#241C16] rounded-lg">
                      <div>
                        <p className="font-medium text-gray-900 dark:text-white text-sm">{sr.name}</p>
                        {sr.nameLocalized?.te && (
                          <p className="text-xs text-gray-400 dark:text-[#E8DDD0]/40">{sr.nameLocalized.te}</p>
                        )}
                        {sr.description && (
                          <p className="text-xs text-gray-500 dark:text-[#E8DDD0]/60 mt-1">{sr.description}</p>
                        )}
                      </div>
                ...
                <p className="text-sm text-gray-400 dark:text-[#E8DDD0]/40 text-center py-6">No sub-rituals</p>
              )}
            </CardContent>
          </Card>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-700 dark:text-[#E8DDD0]/80">Active</span>
                <button
                  onClick={() => setIsActive(!isActive)}
                  className={`relative w-11 h-6 rounded-full transition-colors ${isActive ? 'bg-[#FF6B00]' : 'bg-gray-300 dark:bg-[#E8DDD0]/30'}`}
                >
                  <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform ${isActive ? 'translate-x-5' : ''}`} />
                </button>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-700 dark:text-[#E8DDD0]/80">Show as Banner</span>
                <button
                  onClick={() => setIsBanner(!isBanner)}
                  className={`relative w-11 h-6 rounded-full transition-colors ${isBanner ? 'bg-[#FF6B00]' : 'bg-gray-300 dark:bg-[#E8DDD0]/30'}`}
                >
